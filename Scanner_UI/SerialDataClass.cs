...
namespace PhistonUI
{
    public sealed class SerialDataClass
    {
   
        // constructor
        private SerialDataClass()
        {
            comPortStart();
        }
        ...
        public static void TxTimer_Tick(object source, object e)
        {
            if (tx_busy)
                return;
            // This is used to space the system data requests
            if (Globals.tick_count < 100)
                ++Globals.tick_count;
        ...
        public async Task comPortStart()
        {
            ...
            // This will select the native Pi serial port when in the Arm config, and the Silicon Labs adapter when on the PC
            // This added code is necessary to distinguish between the port that talks to the controller and the port that talks to the PC
            try
            {
                ...
                {
                    matchString = "BCM";                    // Pi has been targeted, look for the BCM port to talk to the controller

                    // Open the port that talks to the controller
                    for (int i = 0; i < size; i++)
                    {
                        if (dis[i].Id.Contains(matchString))
                        {
                            port_num = (int?)i;
                        }
                    }
                ...
                {
                    matchString = "Silicon Labs CP210x USB";  // PC has been targeted, look for specific adapter used during debug

                    // Open the port that talks to the controller
                    for (int i = 0; i < size; i++)
                    {
                        if (dis[i].Name.Contains(matchString))
                        {
                            port_num = (int?)i;
                        }
                    }
                ...
                serialPort = await SerialDevice.FromIdAsync(dis[(int)port_num].Id);
        ...
        private async void RxProcess(byte data)
        {
            int c, clast;
